...
exp_name=${exp_name:-"64-large-img-W-${enable_weighted_sample}-Instruction-ne-${num_environment_per_step}-model-${model_name}-lr-${lr}-bsz-${train_prompt_bsz}-n_resp-${n_resp_per_prompt}-mini-${train_prompt_mini_bsz}"}
...
TRAIN_FILE=${TRAIN_FILE:-"${RAY_DATA_HOME}/SCALER-data/train/SCALER-64.json"}
