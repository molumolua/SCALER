...
enable_weighted_sample=True

entropy_coeff=0

exp_name=${exp_name:-"target-new-w-${enable_weighted_sample}-ne-${num_environment_per_step}-model-${model_name}-lr-${lr}-bsz-${train_prompt_bsz}-n_resp-${n_resp_per_prompt}-mini-${train_prompt_mini_bsz}"}
...
TRAIN_FILE=${TRAIN_FILE:-"/inspire/hdd/global_user/xucaijun-253108120121/Code/FORGE/FORGE_train_data_slope_age_new_target.json"}
