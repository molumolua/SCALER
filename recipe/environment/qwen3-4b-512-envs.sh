...
exp_name=${exp_name:-"windows-10-512-large-img-W-${enable_weighted_sample}-Instruction-ne-${num_environment_per_step}-model-${model_name}-lr-${lr}-bsz-${train_prompt_bsz}-n_resp-${n_resp_per_prompt}-mini-${train_prompt_mini_bsz}"}
...
TRAIN_FILE=${TRAIN_FILE:-"/inspire/hdd/global_user/xucaijun-253108120121/Code/FORGE_THIRD/FORGE_train_data_large_img_512.json"}
...
PYTHONUNBUFFERED=1 python3 -m recipe.environment.main_dapo \
    ...
    +trainer.windows_continous_zero_correct_limit=${windows_continous_zero_correct_limit} \
    +trainer.windows_continous_max_distance_limit=${windows_continous_max_distance_limit}
